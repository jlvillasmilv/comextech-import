...
class CreateLocalWarehousesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('local_warehouses', function (Blueprint $table) {
            $table->id();
            $table->foreignId('application_id')->references('id')->on('applications')->onDelete('SET NULL');
            $table->foreignId('warehouse_id')->references('id')->on('warehouses')->onDelete('SET NULL');
