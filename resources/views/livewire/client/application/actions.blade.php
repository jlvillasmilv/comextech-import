<div class="grid grid-cols-2 gap-0 place-items-center">
    ...
    @if ($application->status->modify and $application->payment->status != 'Paid')
        <button title="Actualizar el costo de la solicitud {{ $application->code }} al tipo de cambio vigente"
            wire:click="$emit('setApplication', '{{ base64_encode($application->id) }}', 'setApplicationSummary')"
    ...
    @if ($application->status->name == 'Borrador')
        <button wire:click="$emit('setApplication', '{{ base64_encode($application->id) }}','delete')"
    ...
    @if ($application->tco_clp > 0)
        @if (!$application->status->client_modify)
            @if (round($application->tco_clp - $application->applicationPayment->sum('total'), 0) > 0)
                <!-- boton modal here -->
                <button title="Pagar Solicitud {{ $application->code }}" type="button"
