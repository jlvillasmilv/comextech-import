<template>
  <div v-if="mode" class="w-full">
    <div class="flex flex-wrap justify-center">
      <div class="sm:px-2 flex w-10/12">
        <h2 class="text-blue-1300 text-xl font-medium mb-3">Usuarios</h2>
      </div>
        ...
        <tbody v-if="users.length > 0">
          ...
          >
            ...
            <td class="px-2 py-1 text-sm">
              <p v-for="rol in item.roles" :key="rol.name">{{ rol.name }}</p>
            </td>
            <td class="px-2 py-1 text-sm">
              <a
                class="
  ...
  <form v-else class="flex flex-col relative sm:w-full md:w-full lg:w-7/12 pr-3">
    ...
    <div class="flex flex-row flex-wrap mb-4 -mr-3.5 -ml-3.5">
      ...
      <div class="flex flex-col relative sm:w-full md:w-full lg:w-6/12 px-3.5">
        <label class="text-gray-500 mb-2">Correo electrónico </label>
        <input
          type="email"
          class="
            block
...
export default {
  ...
  methods: {
    ...
    viewEditForm(item) {
      ...
      item.roles.map((rol) => (this.userCompany.role_id = rol.id));
      this.mode = !this.mode;
    },
    typeAction(payload) {
      this.getUsers();
